...
:UpdateFerium
	...
	powershell.exe -Command "Invoke-WebRequest -Headers @{'Cache-Control'='no-cache, no-store, must-revalidate'} $FeriumApp -o ferium.exe"
    cd ..
exit /b 0
...
:UpdateDatas
	...
	powershell.exe -Command "Invoke-WebRequest -Headers @{'Cache-Control'='no-cache, no-store, must-revalidate'} $ListVersion -o version.txt"
	powershell.exe -Command "Invoke-WebRequest -Headers @{'Cache-Control'='no-cache, no-store, must-revalidate'} $ListModrinth -o modrinth.txt"
	powershell.exe -Command "Invoke-WebRequest -Headers @{'Cache-Control'='no-cache, no-store, must-revalidate'} $ListCurseforge -o curseforge.txt"
    cd ..
exit /b 0
