...
:UpdateFerium
    ...
    cd tools\
		...
		powershell.exe -ExecutionPolicy Unrestricted -WindowStyle Maximized -Command "wget --no-cache $FeriumApp -o ferium.exe"
    cd ..
exit /b 0
...
:UpdateDatas
    ...
    cd datas\
		...
		powershell.exe -ExecutionPolicy Unrestricted -WindowStyle Maximized -Command "wget --no-cache $ListVersion -o version.txt"
		powershell.exe -ExecutionPolicy Unrestricted -WindowStyle Maximized -Command "wget --no-cache $ListModrinth -o modrinth.txt"
		powershell.exe -ExecutionPolicy Unrestricted -WindowStyle Maximized -Command "wget --no-cache $ListCurseforge -o curseforge.txt"
    cd ..
exit /b 0
